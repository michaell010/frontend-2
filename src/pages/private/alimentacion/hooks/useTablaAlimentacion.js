// src/pages/private/alimentacion/hooks/useTablaAlimentacion.js
...
export default function useTablaAlimentacion(registros) {
  ...
  const filtrados = useMemo(() => {
    let arr = registros.filter(r => {
      ...
      const coincideTipo      = !filtros.tipos.length       || filtros.tipos.includes(r.tipo_alimento);
      const coincideTipoAnimal = !filtros.tiposAnimal.length || filtros.tiposAnimal.includes(r.tipo_animal);
    ...
    if (sortCol) {
      arr = [...arr].sort((a, b) => {
        ...
        if (sortCol === "animal") { va = a.animal?.nombre ?? ""; vb = b.animal?.nombre ?? ""; }
